...
<%
	try{
		...
		String sqlStr = "SELECT * FROM item where item_id < 3";
		...
		while (rset.next()) {
	        ...
	        String currentUri = request.getRequestURI();
	       	if(session.getAttribute("name") != null){
	       		try{
					...
					pstmt.setInt(2,rset.getInt("item_id"));
					ResultSet result = pstmt.executeQuery();
					if (result.next()) {
				        String item_id = rset.getString("item_id");
				       	item_id.equals(rset.getInt("item_id"));
	        ...
	        %>
	        	<td id="<%=rset.getInt("item_id")%>">
	        		<a class="detaillink" href="Detail.jsp?item=<%=rset.getInt("item_id")%>">
	        		<div class="product-image"><img src="<%=rset.getString("imgurl")%>"></div></a>
	        		<table id="clickTable">
	        			<tr>
	        				<td><div class="product-info" class="product-name" ><%=rset.getString("name")%></div></td>
	        				<td rowspan="2">
	        					<%if(session.getAttribute("name") == null) {
	        						%>
	        						<img src="image/unlike.png" style="width:40px;float:right;" onclick="alert('로그인이 필요합니다.');">
	        						<%
	        					}else{
	        					%>
	        					<form action="LikeProcess.jsp" method="post">
		        					...
		        					<input type="hidden" name="item_id" value="<%=rset.getInt("item_id")%>">
		        					<input type="hidden" name="islike" value="<%=islike%>">
		        					<input type="image" src="<%=likeImgUrl %>" style="width:40px;float:right;">
		        				</form>
	        					<%
	        					}
	        					%>
	        				<td rowspan="2">
		        				<%if(session.getAttribute("name") == null) {
	        						%>
	        						<img src="image/cart.png" style="width:60px;float:right;" onclick="alert('로그인이 필요합니다.');">
	        						<%
	        					}else{
	        					%>
	        			...
	        			<tr>
	        				<td><div class="product-info" class="product-price"><%=rset.getInt("price")%></div></td>
	        			</tr>
	        		</table>
	        	</td>
	        <%
