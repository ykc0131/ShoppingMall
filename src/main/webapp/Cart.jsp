...
try{	
	...
	String sqlLikeStr = "SELECT * FROM cart left join item on cart.item_id = item.item_id WHERE id=?";
	...
	String currentUri = request.getRequestURI();
	 while(result.next()) {
		 %>
		  <tr>
      		<td><div class="cart-info" class="product-name" ><%=result.getString("name")%></div></td>
      		<td><div class="cart-info" class="product-price"><%=result.getInt("price")%></div></td>
       		<td>
        		<form action="CartDeleteProcess.jsp" method="post" class="btndesign" id="deleteform" onsubmit="if(!confirm('삭제하시겠습니까?')){return false;}">
...
<center>
<br><br><br>
<div style="font-size:20px">총 금액 : <%=totalPrice %></div>
</center>
