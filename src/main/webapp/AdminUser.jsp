...
<%
	try{
		...
		while (rset.next()) {
	        %>
	        	<tr>
		        	<td id="board-num"><%=count++ %></td>
		        	<td id="board-title"><%=rset.getString("login_id")%></td>
		        	<td class="user-name"><%=rset.getString("name")%></td>
		        	<td>
		        		<form action="AdminUserDelete.jsp" method="post" class="btndesign" id="deleteform" onsubmit="if(!confirm('삭제하시겠습니까?')){return false;}">
